# ArcanaGraph - Hackathon Project Context

## What is this?
A multiplayer math dueling game built for DiamondHacks 2026 (36-hour hackathon, magic/wizard theme). Players draw math function curves in the air using hand tracking via webcam. An opponent does the same simultaneously. Drawings are judged against the real curve for accuracy. Higher accuracy = more damage to your opponent. First to deplete the other player's HP wins.

One sentence pitch: "Draw math functions in the air with your hands to cast spells and battle your friends."

## Team
- Luke: computer vision, hand tracking pipeline, scoring engine
- Sean: database, server infrastructure, Digital Ocean droplet
- Diana: frontend, UI/UX
- Adam: spell/equation system, randomization, battle infrastructure (waiting rooms, matchmaking)

## Tech Stack
- **Frontend:** Next.js (React), TypeScript
- **Hand Tracking:** MediaPipe Tasks Vision (@mediapipe/tasks-vision) - runs entirely in browser, 21 hand landmarks per hand at 30fps
- **Game Server:** Node.js with socket.io for real-time multiplayer (same process serves frontend and handles WebSocket game logic)
- **Auth:** Firebase Auth
- **Database:** PostgreSQL (local on the server/droplet)
- **Math Evaluation:** Math.js for parsing and evaluating function expressions
...
Browser (client-side, all the heavy stuff)
├── MediaPipe Hands (hand tracking via webcam)
├── Canvas 2D (drawing surface + graph rendering)
├── Math.js (evaluate target functions)
├── Scoring engine (SDF + centroid alignment)
└── Firebase Auth client
```

## Pages / Routes
- `/login` - Firebase auth login
- `/` - Home with dashboard at bottom
- `/settings` - Light/dark mode, logout, change password (Firebase handles password reset)
- `/play` - Mode selection (solo vs battle), then game view
- `/leaderboard` - Rankings, differentiated by division

## Game Modes
### Battle Mode (PRIORITY)
- Two players matched via waiting room
- Both see the same equation prompt simultaneously
- Both draw in the air with index finger, tracked by MediaPipe
- When both finish (or timer expires), drawings are judged against ground truth
- First to get an equation right damages the other player
- Damage scales based on accuracy score (minimum accuracy threshold required to deal any damage)
- HP system, first to 0 loses
- Multiple rounds per match with different equations

### Solo Mode
- Practice drawing functions
...
- Use @mediapipe/tasks-vision (newer API, not the legacy @mediapipe/hands)
- Track index finger tip (landmark 8) for drawing position
- Detect gestures:
  - Extended index finger = drawing mode (pen down)
  - Fist = stop drawing (pen up)
- Mirror the webcam feed so movements feel natural

### Signal Processing
- Apply a **Kalman filter** to the index finger tip coordinates to remove jitter while preserving responsiveness. The Kalman filter is preferred over simple EMA because it handles variable velocity better (fast strokes stay responsive, slow precise movements get smoothed).
- Kalman filter parameters to start with:
  - Process noise (Q): ~0.01
  - Measurement noise (R): ~0.1
  - Tune these based on testing

### Drawing
- Capture filtered (x, y) points as the player draws
- Apply **Catmull-Rom spline interpolation** to the captured points before rendering and scoring. This ensures smooth curves even when hand tracking gives sparse points during fast movement.
- Render the player's stroke on a Canvas 2D overlay

## Scoring Engine

### Ground Truth Rendering
- Use **Math.js** to evaluate the target function: `evaluate('sin(x)', { x })`
- Sample the function at fine intervals (step size 0.05) across the visible x range
- Render on Canvas 2D with `lineTo` between sample points (200-400 points = visually smooth curve)
- Show axis gridlines and labels so players have spatial reference

### Scoring Algorithm (Hybrid: Shape + Position)

**Position Score (30% weight):**
1. Compute centroid of player's drawn points
2. Compute centroid of ground truth curve samples
3. Distance between centroids, normalized by canvas size
4. Closer centroids = higher position score

**Shape Score (70% weight):**
1. Translate both curves so their centroids are at the origin
2. Normalize scale (divide by standard deviation or max extent) with a clamp so tiny drawings can't cheat (only normalize if scale ratio is between 0.5x and 2x)
3. Compute **Signed Distance Function**: for each point on the player's curve, find the minimum distance to the nearest point on the ground truth curve
4. Average absolute SDF distance across all player points
5. Lower average distance = higher shape score

**Time Bonus (small multiplier):**
- Finishing faster gives a small bonus, maybe 10-20% of final score
- Encourages speed but accuracy dominates

**Final Score:**
```
score = (shape_score * 0.7 + position_score * 0.3) * time_multiplier * 100
```

**Damage Calculation:**
- Minimum accuracy threshold (e.g., 40%) required to deal any damage
- Below threshold = 0 damage (you "fizzled" the spell)
- Above threshold: damage scales linearly with score
- First player to submit a passing score in a round deals damage, incentivizing speed

### SDF Implementation Note
- Naive O(n*m) comparison is fine for hackathon. With ~300 points per curve, it runs in under 1ms.
- For each drawn point, iterate all truth points, find minimum distance. No need for KD-trees.

## Socket.io Game Flow

### Room/Match Lifecycle
1. Player creates a room, gets a 4-6 character room code
2. Opponent joins with code
3. Server sends countdown (3, 2, 1, GO)
4. Server sends the equation prompt to both players simultaneously
5. Players draw locally. No stroke streaming during drawing (simpler, more dramatic reveal).
6. When a player finishes: client computes score locally, sends `{ score, strokeData }` to server
7. Server validates that both players get the same prompt, tracks who finished first
8. When both finish (or time limit hits): server sends both players' results to both clients
9. Reveal screen: overlay both drawings on the ground truth, show scores, animate HP damage
10. Server sends next round prompt after a short delay
11. Repeat until someone's HP hits 0
...
rooms: {
  [roomCode]: {
    ...
    hp: { [socketId]: number },
    status: 'waiting' | 'countdown' | 'drawing' | 'reveal' | 'finished'
  }
}
```

## Equation Bank
Organize equations by difficulty tier:
...
Randomize within tiers. Both players always get the same equation. Difficulty can escalate as rounds progress.

## Reach Goals (in priority order)
1. Double-handed mode (two hands drawing simultaneously)
2. Country/state shape drawing mode
3. Cursive writing mode
4. Kids mode with simple shapes (circles, triangles, stars)

## Leaderboard / Database Schema (PostgreSQL)
```sql
users (
  id, firebase_uid, display_name, created_at
)
matches (
  id, player1_id, player2_id, winner_id, created_at
)
rounds (
  id, match_id, equation, player1_score, player2_score, player1_time, player2_time
)
leaderboard view:
  wins, losses, win_rate, avg_accuracy, division/rank
```

Divisions could be based on win count or ELO-style rating.

## Coding Guidelines

### General Principles
- Ship over polish. Working demo in front of judges is the only goal.
...
- Don't build elaborate error handling systems. console.error is fine.

### Comments
- Comments explain WHY, not WHAT.
- No dead code. No commented-out blocks. Delete or use it.

### Dependencies
- Before installing a new package, consider if you can do it in 20 lines instead.
- Prefer well-known, well-documented libraries.
- Pin versions in package.json.

### Performance (CRITICAL for this project)
- **Hot loop rule:** MediaPipe runs every frame at 30fps. The hand tracking callback, Kalman filter, and drawing code must be lean. No DOM queries per frame. No object allocations in the tracking loop. Pre-allocate buffers. Cache DOM references outside the loop.
- Scoring computation happens once when the player finishes, not per-frame. It can be slower.
- Canvas rendering is fast. Don't optimize it unless there's a visible problem.

### Frontend / UI
- Use a single spacing scale (multiples of 4px) for all margins, padding, gaps.
- Optimize for laptop screen (1920x1080 or 1440x900). Mobile doesn't matter for a hackathon demo.
- The game view is the most important screen. Prioritize making it feel good over making other pages pretty.
- Dark theme by default (looks better for demos and projectors, also fits the wizard theme).
- Wizard/magic aesthetic: think dark purples, deep blues, gold accents, glowing effects on the drawn curves.

### Git
- Commit often with short descriptive messages.
- Work on main branch. Don't bother with feature branches.
- Coordinate over Discord/in-person to avoid merge conflicts.

## Demo Strategy (2 minutes)
1. "This is ArcanaGraph. You draw math functions in the air to cast spells and battle your friends."
2. Two team members step up to two laptops
3. They battle live. Draw sin(x), draw x², draw something harder. Crowd watches the HP bars go down.
4. Someone wins. Crowd reacts.
5. "Every equation is scored in real time using computer vision and signed distance functions. Hand a judge a laptop and let them try."
6. Judge tries solo mode. They smile when they see their wobbly sine wave overlaid on the real one.

The live battle IS the demo. If it's fun to watch two people play, you win.
