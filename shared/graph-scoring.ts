// Shared scoring and equation logic — used by both frontend and backend.
...
// Wire-safe equation descriptor (no compiled fn, safe for JSON/socket)
export type SerializableEquation = {
  type: "explicit" | "parametric_circle";
  jsExpr: string;
  label: string;
  latex: string;
  hasTurningPoints: boolean;
...
function buildEquationParts(family: EquationFamily) {
  ...
  return { values, label, latex, jsExpr };
}

export function generateEquation(family: EquationFamily): EquationConfig {
  const { values, label, latex, jsExpr } = buildEquationParts(family);
...
// Returns both the compiled config (for scoring) and a serializable descriptor (for the wire)
export function generateEquationWithDescriptor(family: EquationFamily): {
  config: EquationConfig;
  descriptor: SerializableEquation;
} {
  const { values, label, latex, jsExpr } = buildEquationParts(family);
  const hasTurningPoints = family.hasTurningPoints === "true";
...
// Reconstruct an EquationConfig from a serializable descriptor
export function equationConfigFromDescriptor(desc: SerializableEquation): EquationConfig {
  if (desc.type === "parametric_circle") {
    ...
    return {
      ...
      label: desc.label,
      latex: desc.latex,
      hasTurningPoints: desc.hasTurningPoints,
      fn: null,
    };
  }

  const fn = new Function("x", `return ${desc.jsExpr};`) as (x: number) => number;
...
// Select a random equation and return both config + descriptor
export function selectRandomEquationWithDescriptor(
  ...
  if (!pool.length) {
    ...
    const descriptor: SerializableEquation = {
      type: "explicit",
      jsExpr: "x",
      label: "y = x",
      latex: "y = x",
  ...
  const family = pool[Math.floor(Math.random() * pool.length)];
  return generateEquationWithDescriptor(family);
}

export function sampleCurvePoints(equationConfig: EquationConfig, step?: number) {
  const width = 640;
